    ...
    for period in data["PERIOD"].unique():
        ...
        if len(all_id) == 10:
            ...
            while retry < args.get("retry", 5):
                try:
                    print('getting players from nba.com')
...
def players_name(id_players: Union[pd.Series, np.ndarray, List],
                       player_data: Optional[pd.DataFrame] = pd.read_csv(current_directory / 'player_data.csv'), **kwargs: Dict[str, float]) -> List:
